...
class DQN:
    ...
    def __init__(self, session, width, height, n_action):
        ...
        self.input_X = tf.placeholder(tf.float32, [None, width, height, self.STATE_LEN]) #게임의 상태
        self.input_A = tf.placeholder(tf.int64, [None]) #상태를 만들어낸 액션의 값
        self.input_Y = tf.placeholder(tf.float32, [None]) #손실값 계산에 사용하 ㄹ값
